...
def run_etl(df: pd.DataFrame, include_target: bool = True) -> pd.DataFrame:
    ...
    # Drop rows with NaN only in essential columns (not extras like Dividend)
    _essential = [
        ...
        "volume_sma_10", "volume_ratio", "atr_14",
    ]
    if include_target:
        _essential.append("target")
    _check_cols = [c for c in _essential if c in df.columns]
    df = df.dropna(subset=_check_cols).reset_index(drop=True)
