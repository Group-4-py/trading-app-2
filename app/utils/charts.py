...
def confusion_matrix_chart(y_true: np.ndarray, y_pred: np.ndarray) -> go.Figure:
    ...
    Uses a custom z matrix to color diagonal cells (correct) green and
    off-diagonal cells (incorrect) red — all with visible base shades so
    no cell blends into the dark background.
    """
    ...
    # Encode diagonal (correct) as positive, off-diagonal (wrong) as negative
    # so we can map them to separate ends of the colorscale
    max_val = max(cm.max(), 1)
    z_colored = [
        [ cm[0][0] / max_val,  -cm[0][1] / max_val],  # TN positive, FP negative
        [-cm[1][0] / max_val,   cm[1][1] / max_val],  # FN negative, TP positive
    ]

    # Colorscale: -1 (wrong, high) → red  |  0 (low) → dark visible mid  |  +1 (correct, high) → green
    colorscale = [
        [0.0,  "#8b1a1a"],  # strong wrong → deep red (always visible)
        [0.35, "#4a1515"],  # mild wrong   → dark red
        [0.5,  "#1e2538"],  # near-zero    → dark slate (visible but neutral)
        [0.65, "#1a4a2e"],  # mild correct → dark green
        [1.0,  "#00c853"],  # strong correct → bright green
    ]
    ...
    fig = go.Figure(go.Heatmap(
        ...
        text=text,
        texttemplate="%{text}",
        textfont=dict(color="white", size=15, family=FONT),
...
def prediction_distribution_chart(pred_df: pd.DataFrame) -> go.Figure:
    """
    Distribution of P(UP) probabilities, split at the 0.50 decision boundary.
    Bars below 0.50 are red (DOWN predictions), above 0.50 are green (UP predictions).
    This directly corresponds to the binary predictions shown in the timeline.
    """
    fig = go.Figure()

    prob_up = pred_df["prob_up"].dropna()
    n_bins = 20
    bin_min, bin_max = prob_up.min(), prob_up.max()
    # Ensure boundary spans at least 0–1 for context
    bin_min = min(bin_min, 0.3)
    bin_max = max(bin_max, 0.7)
    bin_size = (bin_max - bin_min) / n_bins

    # DOWN region (prob_up < 0.50)
    down_vals = prob_up[prob_up < 0.50]
    fig.add_trace(go.Histogram(
        x=down_vals, name="Predict DOWN",
        marker_color=RED, opacity=0.75,
        xbins=dict(start=bin_min, end=0.50, size=bin_size),
    ))

    # UP region (prob_up >= 0.50)
    up_vals = prob_up[prob_up >= 0.50]
    fig.add_trace(go.Histogram(
        x=up_vals, name="Predict UP",
        marker_color=GREEN, opacity=0.75,
        xbins=dict(start=0.50, end=bin_max, size=bin_size),
    ))
    ...
    n_up = (prob_up >= 0.50).sum()
    n_down = (prob_up < 0.50).sum()
    total = len(prob_up)
    ...
    fig.update_layout(
        ...
        annotations=[dict(
            ...
            text=f"UP: {n_up} ({n_up/total:.0%})  |  DOWN: {n_down} ({n_down/total:.0%})",
            showarrow=False,
            font=dict(color=TEXT_COLOR, size=11),
            bgcolor="rgba(26,31,46,0.8)",
        )],
    )
    return _apply_defaults(fig, height=300, title="Probability Distribution")
